  ...
  return (
    <button
      ...
      className={cn(
        ...
        "bg-foreground text-white shadow-xl shadow-black/5",
        !isSubmitting &&
          !isSuccess &&
          "hover:bg-brand hover:shadow-brand/20 hover:px-14",
        (isSubmitting || isSuccess) && "cursor-default px-14 opacity-100"
      )}
    >
      <div
        className={cn(
          "bg-brand/20 absolute inset-0 origin-left transition-transform duration-[3s] ease-out",
      ...
      <div className="relative z-10 flex items-center justify-center gap-3 text-sm font-bold tracking-widest uppercase">
        {isSuccess ? (
          <>
            ...
            <div className="text-brand animate-in zoom-in flex h-5 w-5 items-center justify-center rounded-full bg-white duration-300">
