 ...
 * @description Infinite scrolling marquee for client logos.
 * Features CSS-driven animation, pause-on-hover, and edge-fading gradients.
 * @dependencies
...
export const LogoMarquee = ({ brands }: LogoMarqueeProps) => {
  ...
  return (
    <RevealWrapper asChild>
      <div className="w-full max-w-full overflow-hidden py-12 transition-colors duration-300">
        {/* Section Header */}
        <div className="container mx-auto mb-8 px-4 sm:px-6 lg:px-8">
          <p className="reveal-item text-clay text-center text-[10px] font-bold tracking-widest uppercase transition-colors duration-300 dark:text-zinc-500">
          ...
          {[1, 2].map((trackId) => (
            ...
            >
              {trackBrands.map((brand, i) => (
                <div
                  key={`${trackId}-${i}`}
                  className="relative h-24 w-24 shrink-0 cursor-default overflow-hidden rounded-2xl transition-all duration-300 group-hover/marquee:opacity-40 hover:opacity-100! md:h-36 md:w-36"
                >
                  <Image
                    src={brand.logo as string}
                    alt={brand.name || "Client Brand Logo"}
                    fill
                    // Removed all color-altering filters; maintaining raw brand assets
